...
pie_allItems.add_trace(go.Pie(values = all_counts.values,
...
auto_brands = auto.Brand.value_counts().head(10)
brand_issues = []
...
bar1_auto = go.Figure(data=[go.Bar(x = auto_brands.values,
                                   y = auto_brands.index,
...
bar2_auto = go.Figure(data=[go.Bar(x = auto_gr.values,
...
line_food.update_layout(xaxis_title = 'Year', 
                        ...
                        title ='Recalled Food Items in Canada from 2012 to 2022',
                        hovermode="x")
...
foodIssue = food['Food_Issue'].value_counts()
...
microbiological = microbiological['Sub Issue'].value_counts()
...
allergen = allergen['Sub Issue'].value_counts()


# Plot Pie Charts - Food
pie_FoodIssue = go.Figure(go.Pie(values = foodIssue.values,
...
pie_Microbiological = go.Figure(go.Pie(values = microbiological.values,
...
pie_Allergen = go.Figure(go.Pie(values = allergen.values,
...
bar1_consumer = go.Figure(data=[go.Bar(x = consumer_issues,
...
consumer_categories = consumer['Category'].value_counts().head(10)
...
# Bar plot with the most common issue for each brand
bar2_consumer = go.Figure(data=[go.Bar(x = consumer_categories.values,
                                       y = consumer_categories.index, 
                                       orientation = 'h',
                                       text = [issue for issue in consumer_issues] ,
                                       textposition = 'inside',
                                       marker = {'color': px.colors.diverging.Geyser})])
...
bar_medical = go.Figure(data=[go.Bar(x = medical_gr,
                                     y = medical_gr.index,
                                     orientation = 'h', 
                                     marker = {'color': px.colors.diverging.Geyser})])
